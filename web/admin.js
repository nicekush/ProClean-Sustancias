...
function render(){
 ...
 $('#metrics').innerHTML=Object.entries(names).map(([k,n])=>`<div class="metric">${n}<b>${counts[k]}</b></div>`).join('')+`<div class="metric">% bloqueadas sobre finalizadas<b>${counts.approved+counts.blocked?Math.round(100*counts.blocked/(counts.approved+counts.blocked)):0}%</b></div>`;
 $('#rows').innerHTML=rows.map(r=>`<tr><td>${time(r.started_at)}</td><td>${esc(r.operator_name)}</td><td>${esc(r.plate)}</td><td>${r.answers.area==='wet'?'Húmeda':r.answers.area==='dry'?'Seca':'—'}</td><td class="status-${r.status}">${names[r.status]}</td><td><button data-id="${r.id}">Ver registro</button></td></tr>`).join('');$('#more').hidden=!cursor;
